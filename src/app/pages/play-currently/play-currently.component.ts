...
@Component({
  ...
  templateUrl: './play-currently.component.html',
  styleUrl: './play-currently.component.scss'
})
export class PlayCurrentlyComponent implements OnInit {
  list: List | undefined;
  ...
  ngOnInit() {
    this.userService.getLists().subscribe({
      next: (lists: any[]) => {
        this.list = lists.find((list: { name: string; }) => list.name === 'Currently Playing');
